...
def naoInitForTest(logName="someTest"):
    """开始前机器人的初始化"""
    try:
        loadModule()
        logConfig(logName)
...
def trackBallNoHead(headPitch=0):
    ...
    while True:
        ...
        if len(g_tracker.getTargetPosition(2)) == 3:
            # g_tts.say("got it")
            # 以脚部坐标系获取坐标 number 2 maybe mean the fit,0 mean head,1 mean body
            ...
            distance = getRealDisForBall(coord[0], coord[1], coord[2])
...
# -------------找到球的一些行为--------------
def actionAfterFirstFindBall(headYawAngle):
    """
    找到球后，正对球，调整距离
    ...
    if moveDistance > 40:
        move(30)
        distance, Angle, ballCoord = trackBall()
        actionAfterFirstFindBall(Angle)
    else:
        move(x=moveDistance)
        # 再次正对
        time.sleep(0.5)
...
def findAndHitBall(changci=1):
    ...
    for times in xrange(1):
        ...
        if distance != -1:
            # 5、调整与球之间的距离
            actionAfterFirstFindBall(headYawAngle)
        ...
        if distance != -1:
            ...
            if not isFind:
                if changci != 2:
                    ...
                    robotToLandmarkData, isFind = searchLandmarkInLeft()
            # for i in range(2): distanceToLandmark += robotToLandmarkData[0] robotToLandmarkData, isFind =
            # searchLandmark() logging.debug("NAO与landMark的距离:  " + str(robotToLandmarkData[0]) + "  角度:  " + str(
            # robotToLandmarkData[1])) robotToLandmarkData[0] = distanceToLandmark / 3 6.1、找landmark
            if isFind:
                # 7、调整角度
                # 7.1 重新计算与球的距离,并获取球与landmark的距离
                distance2Ball, headYawAngle, ballCoord = trackBall(18)
                ...
                if adjustY > 25 or adjustY < -25:
                    ...
                    # 重新获取数据
                    # distance2Ball_2, headYawAngle_2, ballCoord_2 = trackBall(18)
                    # if headYawAngle < 0: robotToLandmarkData_2, isFind = searchLandmarkInLeft() distance2Ball_2+=15
                    #  else: robotToLandmarkData_2, isFind = searchLandmarkInRight() distance2Ball_2+=5.5 if isFind:
                    # logging.info("-----------第一种------------") distanceFromBall2Mark_2 = calDistanceFromBall2Mark(
                    # distance2Ball_2, robotToLandmarkData_2[0], robotToLandmarkData_2[1]) adjustY_2, adjustTheta_2 =
                    #  calAdjustY(distance2Ball_2, robotToLandmarkData_2[0], robotToLandmarkData_2[1],
                    # distanceFromBall2Mark_2) adjustX = distance2Ball_2 - distance2Ball_2 * math.sin(abs(
                    # adjustTheta_2)) logging.info("adjustX:::" + str(adjustX)) logging.info(
                    # "-----------第二种------------") L = math.sqrt(distance2Ball_2**2 - ballCoord_2[1]**2)
                    # distanceFromBall2Mark_2 = calDistanceFromBall2Mark(L, robotToLandmarkData_2[0],
                    # robotToLandmarkData_2[1]) adjustY_2, adjustTheta_2 = calAdjustY(L, robotToLandmarkData_2[0],
                    # robotToLandmarkData_2[1], distanceFromBall2Mark_2) adjustX_2 = L -  L * math.sin(abs(
                    # adjustTheta_2)) logging.info("adjustX_2:::"+str(adjustX_2))

                    g_motion.angleInterpolationWithSpeed("HeadPitch", 0.0, 0.1)
                    g_motion.angleInterpolationWithSpeed("HeadYaw", 0.0, 0.1)
                    # times = times + 1 for i in xrange(3): robotToLandmarkData, isFind =   searchLandmark()
                    # logging.info("最后的NAO与landMark的距离:  " + str(robotToLandmarkData[0]) + "  角度:  " + str(
                    # robotToLandmarkData[1])) logging.info("球与landmark的距离: " + str(distanceFromBall2Mark))
                    # distance2Ball, headYawAngle, ballCoord = trackBall(18) distanceFromBall2Mark =
                    # calDistanceFromBall2Mark(distance2Ball, robotToLandmarkData[0], robotToLandmarkData[1])
                    #
                    # # 7.2、计算需移动的y值
        ...
        if distance != -1:
            ...
            if isFind:
                ...
                if adjustY > 25 or adjustY < -25:
                    ...
                    # 7.4 、调整至击球点
                    firstClosingForHitBall(robotToLandmarkData[1])
                    # 重新获取数据 distance2Ball_2, headYawAngle_2, ballCoord_2 = trackBall(18) if headYawAngle < 0:
                    # robotToLandmarkData_2, isFind = searchLandmarkInLeft() distance2Ball_2+=15 else:
                    # robotToLandmarkData_2, isFind = searchLandmarkInRight() distance2Ball_2+=5.5 if isFind:
                    # logging.info("-----------第一种------------") distanceFromBall2Mark_2 = calDistanceFromBall2Mark(
                    # distance2Ball_2, robotToLandmarkData_2[0], robotToLandmarkData_2[1]) adjustY_2, adjustTheta_2 =
                    #  calAdjustY(distance2Ball_2, robotToLandmarkData_2[0], robotToLandmarkData_2[1],
                    # distanceFromBall2Mark_2) adjustX = distance2Ball_2 - distance2Ball_2 * math.sin(abs(
...
# ---------------------------------一些修改后废弃的方法------------------
def naoInit_old(logName="no log name"):
    """开始前机器人的初始化
    """
    try:
        loadModule()
...
def naoInit2(logName="no log name"):
    """开始前机器人的初始化"""
    try:
        loadModule()
        logConfig(logName)
